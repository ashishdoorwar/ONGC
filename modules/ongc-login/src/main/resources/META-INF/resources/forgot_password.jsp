...
<portlet:actionURL name="/login/forgot_password" var="forgotPasswordURL">
	<portlet:param name="mvcRenderCommandName"
		value="/login/forgot_password" />
</portlet:actionURL>

<div class="main-login">
			<div class="login-box">
			<div class="border-0 rounded-0">
			<div class="logo"><img src="${themeDisplay.getPathThemeRoot()}/images/logo.png" alt="Bata Logo">
			</div>
	<aui:form action="<%=forgotPasswordURL%>" method="post" name="fm">
		...
		<liferay-ui:error exception="<%=NoSuchUserException.class%>"
			message='<%="the-" + TextFormatter.format(HtmlUtil.escape(authType), TextFormatter.K)
		...
		<liferay-ui:error
			exception="<%=SendPasswordException.MustBeEnabled.class%>"
		...
		<aui:fieldset>
			<div class="login-text mb-4" style="margin-top:20px"><span>FORGOT PASSWORD</span> </div>
			...
			<c:choose>
				<c:when test="<%=user2 == null%>">
					...
					<aui:input label="User Id" name="<%=loginParameter%>" size="30" type="text" value="<%=loginValue%>">
						<aui:validator name="required" />
					</aui:input>
				...
				<c:when
					...
					<c:if
						test="<%=Validator.isNotNull(user2.getReminderQueryQuestion())
										&& Validator.isNotNull(user2.getReminderQueryAnswer())%>">
						...
						<div class="alert alert-info">
							<liferay-ui:message arguments="<%=HtmlUtil.escape(login)%>"
								key="a-new-password-will-be-sent-to-x-if-you-can-correctly-answer-the-following-question"
								translateArguments="<%=false%>" />
						</div>
						...
						<c:otherwise>
							...
							<aui:button-row>
								<aui:button type="submit"
									value='<%=company.isSendPasswordResetLink() ? "send-password-reset-link"
												: "send-new-password"%>' />
							</aui:button-row>
						</c:otherwise>
					</c:choose>
...
<style>
.navigation .taglib-icon-list li:first-child, .reference-mark, header, footer, banner, .portlet-icon-back, .portlet-layout .portlet-header, .portlet-title-text{display:none !important;}
.navigation .taglib-icon-list{text-align: center; font-size: 14px; margin-top: 10px}
.navigation .taglib-icon-list li{float: none}
body {
  background-color: transparent;
  background-image: url("${themeDisplay.getPathThemeRoot()}/images/Bata_login.jpg");
...
main#content .d-flex.justify-content-end.mb-2{margin-bottom: 0 !important}
</style>