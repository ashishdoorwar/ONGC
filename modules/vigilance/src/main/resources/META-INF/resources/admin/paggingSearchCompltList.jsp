...
String fileName = complaintAttachment.getFileName().substring(0, complaintAttachment.getFileName().indexOf("."));%>
		    ...
		    <% } } %>
		        <%-- <% for(int i=0;i<lst.size();i++){ if(i==0) out.println("<a href='/wps/PA_Vigilance/admin/downloadJsp.jsp?attachmentId="+lst.get(i).getAttachmentId()+"'>"+getTreamFileName(lst.get(i).getFileName())+"</a>"); else out.println(" | <a href='/wps/PA_Vigilance/admin/downloadJsp.jsp?attachmentId="+lst.get(i).getAttachmentId()+"'>"+getTreamFileName(lst.get(i).getFileName())+"</a>");}%> --%>
		        </td>
		        ...
		        <td><span onclick="complaintreadUpdate('<%=vc.getComplaintNo() %>','<%= vc.getStatus() %>')" class="lnk" > <%= vc.getStatus() %></span></td>
